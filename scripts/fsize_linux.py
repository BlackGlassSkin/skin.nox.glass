...
#folder = re.findall('[a-zA-Z0-9]', 'total_mem')[0]
total_mem = 'Total Mem:' + '[B]'" %.2f GiB"'[/B]' % (total_mem/gb)
used_mem = 'Used Mem:' + '[B]'" %.2f GiB"'[/B]' % (used_mem/gb)
free_mem = 'Free Mem:' + '[B]'" %.2f GiB"'[/B]' % (free_mem/gb)

#disk = os.statvfs("/")
#
#totalBytes = float(disk.f_bsize*disk.f_blocks)
#totalBytes = 'total space: %.2f GB' % (totalBytes/BytesPerGiB)
#
#totalUsedSpace = float(disk.f_bsize*(disk.f_blocks-disk.f_bfree))
#totalUsedSpace = 'used space: %.2f GB' % (totalUsedSpace/BytesPerGiB)
#
#totalAvailSpace = float(disk.f_bsize*disk.f_bfree)
#totalAvailSpace = 'available space: %.2f GB' % (totalAvailSpace/BytesPerGiB)
#
#xbmcgui.Window(10000).setProperty('str(totalAvailSpace)', str(totalAvailSpace))
#xbmcgui.Window(10000).setProperty('str(totalUsedSpace)', str(totalUsedSpace))
#xbmcgui.Window(10000).setProperty('str(totalBytes)', str(totalBytes))
